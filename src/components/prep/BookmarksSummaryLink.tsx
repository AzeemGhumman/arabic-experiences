  ...
  if (count === 0) {
    return (
      <div className="rounded-xl border border-dashed border-border/80 bg-card/50 px-3 py-2.5">
        <p className="text-sm font-semibold text-ink">{t("prep.bookmarksTitle")}</p>
        <p className="mt-0.5 text-xs text-muted-foreground">{t("prep.bookmarksEmpty")}</p>
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="block text-sm font-semibold leading-tight">{t("prep.bookmarksTitle")}</span>
        <span className="mt-0.5 block text-xs text-muted-foreground">
          {t("prep.bookmarksSummary", { count })}
        </span>
      </span>
