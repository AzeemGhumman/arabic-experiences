# Study content spec — Arabic for Umrah (historical)

This file is a **content inventory**. Current product names are in [`README-terminology.md`](./README-terminology.md): Study, Topic, Lesson — not Prep, SideMission-as-lesson, or Adventure.

Structured specification for the Study catalog (`Topic` → `Lesson` → study `buildRun`).

**Audience:** content authors and implementers.  
**Code targets:** `topics.ts`, `lessons.ts`, `pools.ts`, `words.ts`, `builders.ts`, `ExperienceScenes.tsx`, `learning-types.ts`.
...
|------|------------------|
| Situations first | Every session links to ≥1 `missionId` or a journey chapter |
| Basic → Advanced | Level 1 = **Basic** (8–15 words, ~4–5 min); Level 2 = **Advanced** (12–20 words, ~5–7 min) |
...
type SideMission = {
  ...
  topicId: PrepTopicId
  level: number                 // 1 = Basic, 2 = Advanced, 3 = Deep (optional)
  levelName: string             // "Basic" | "Advanced" | "Deep"
  description: string           // card subtitle
  missionIds: string[]        // adventures this prep supports
  unlockAfterMissionIds: string[]  // empty = always visible in catalog
...
### 3.2 MissionScene additions (proposed)
...
estimatedMinutes: 5
vocabularyGain: 23
missionIds: [airport-arrival, taxi-hotel, find-haram, enter-haram, begin-tawaf, complete-sai, order-dinner, hajj-bus]
...
estimatedMinutes: 6
vocabularyGain: 18
missionIds: [hajj-bus, taxi-hotel, order-dinner]
...
estimatedMinutes: 4
vocabularyGain: 12
missionIds: [airport-arrival, find-haram, enter-haram, order-dinner, taxi-hotel, lost-group]
unlockAfterMissionIds: []
...
estimatedMinutes: 5
vocabularyGain: 12
missionIds: [hajj-bus, taxi-hotel]
unlockAfterMissionIds: []
...
estimatedMinutes: 4
vocabularyGain: 8
missionIds: [taxi-hotel, hajj-bus, day-madinah]
unlockAfterMissionIds: []
...
estimatedMinutes: 4
vocabularyGain: 8
missionIds: [hajj-bus, something-wrong]
unlockAfterMissionIds: []
...
vocabularyGain: 14
missionIds: [hajj-bus]
unlockAfterMissionIds: [hajj-bus]
...
3. Add scene ids to `MissionScene` + placeholder art in `ExperienceScenes.tsx`.
...
- [ ] `missionIds` populated; unlock rule intentional (empty = always in catalog)
